...
void setupBLE()
{
  ...
  pCharacteristicTank1 = pService->createCharacteristic(BLE_TANK1CHARACTERISTIC_UUID, BLECharacteristic::PROPERTY_READ);
  ...
  pCharacteristicTank1->addDescriptor(new BLE2902());

  pCharacteristicTank2 = pService->createCharacteristic(BLE_TANK2CHARACTERISTIC_UUID, BLECharacteristic::PROPERTY_READ);
  ...
  pCharacteristicTank2->addDescriptor(new BLE2902());
...
void SendSensorValue(BLECharacteristic *pCharacteristic, double value)
{
  ...
  digitalWrite(LED_PIN, LOW);
  delay(100);
  String descriptor = String((const char *)pCharacteristicTank1->getDescriptorByUUID((uint16_t)0x2901)->getValue());
  Serial.println("Sending " + descriptor + " " + String(value));
}
