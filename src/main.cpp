...
void setupBLE()
{
  ...
  pCharacteristicWaterTank = pService->createCharacteristic(BLE_WATER_TANK_CHARACTERISTIC_UUID, BLECharacteristic::PROPERTY_READ | BLECharacteristic::PROPERTY_NOTIFY);
  BLE2902 *pBLE2902WaterTank = new BLE2902();
  pBLE2902WaterTank->setNotifications(true);
  pCharacteristicWaterTank->addDescriptor(pBLE2902WaterTank);
...
void readSensors()
{
  ...
  temperature = random(1000, 7500) / 100.0;
  humidity = random(0, 10000) / 100.0;
}

void sendAllValues()
{
  pCharacteristicWaterTank->setValue(waterTankLevel);
  pCharacteristicWaterTank->notify();
  // digitalWrite(LED_PIN, HIGH);
  // delay(50);
  // digitalWrite(LED_PIN, LOW);
  Serial.println("Sent value: " + String(waterTankLevel) + " to client for characteristic: " + BLE_WATER_TANK_CHARACTERISTIC_UUID);
