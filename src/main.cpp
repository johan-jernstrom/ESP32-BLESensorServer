...
void setupBLE()
{
  ...
  pCharacteristicTank1Percentage = pService->createCharacteristic(
      BLE_TANK1CHARACTERISTIC_UUID,
  ...
  pCharacteristicTank2Percentage = pService->createCharacteristic(
      BLE_TANK2CHARACTERISTIC_UUID,
      BLECharacteristic::PROPERTY_READ |
          BLECharacteristic::PROPERTY_WRITE |
          BLECharacteristic::PROPERTY_WRITE_NR |
          BLECharacteristic::PROPERTY_NOTIFY |
...
void loop()
{
  if (deviceConnected)
  {
    int tank1Percentage = random(0, 100); // simulate tank level
    pCharacteristicTank1Percentage->setValue(tank1Percentage);
    pCharacteristicTank1Percentage->notify();
    Serial.println("Notified: Tank 1 level = " + String(tank1Percentage) + "%");

    int tank2Percentage = random(0, 100); // simulate tank level
    pCharacteristicTank2Percentage->setValue(tank2Percentage);
    pCharacteristicTank2Percentage->notify();
    Serial.println("Notified: Tank 2 level = " + String(tank2Percentage) + "%");
  }
